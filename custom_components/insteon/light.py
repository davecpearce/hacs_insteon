...
class InsteonDimmerEntity(InsteonEntity, LightEntity):
    ...
    def brightness(self) -> int:
        ...
        _LOGGER.debug(
            "Light brightness value for %s address %s group %d brightness %d",
            str(self.name),
            str(self.address),
            self._insteon_device_group.group,
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if ATTR_BRIGHTNESS in kwargs:
            _LOGGER.debug(
                "Light brightness in async_turn_on for %s address %s group %d brightness %d",
                str(self.name),
                str(self.address),
                self._insteon_device_group.group,
            ...
            _LOGGER.debug(
                "Light brightness from ON_LEVEL for %s address %s group %d brightness %d",
                str(self.name),
                str(self.address),
                self._insteon_device_group.group,
                self.get_device_property(ON_LEVEL),
            )
